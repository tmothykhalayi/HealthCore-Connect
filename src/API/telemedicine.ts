...
export const getSessionById = async (
  ...
  const response = await axios.get(
    `${API_BASE_URL}/telemedicine/sessions/${id}`,
  )
  return response.data
}
...
export const createSession = async (
  sessionData: Partial<TelemedicineSession>,
): Promise<TelemedicineSession> => {
  const response = await axios.post(
    `${API_BASE_URL}/telemedicine/sessions`,
    sessionData,
  )
  return response.data
}
...
export const startSession = async (
  ...
  const response = await axios.post(
    `${API_BASE_URL}/telemedicine/sessions/${id}/start`,
  )
  return response.data
}
...
export const endSession = async (id: string): Promise<TelemedicineSession> => {
  const response = await axios.post(
    `${API_BASE_URL}/telemedicine/sessions/${id}/end`,
  )
  return response.data
}
...
export const getSessionRoom = async (
  sessionId: string,
): Promise<{ roomId: string; token: string }> => {
  const response = await axios.get(
    `${API_BASE_URL}/telemedicine/sessions/${sessionId}/room`,
  )
  return response.data
}
